...
class ManagerOrderDetailsScreen extends StatefulWidget {
  ...
  const ManagerOrderDetailsScreen({
    super.key,
    required this.order,
...
class _ManagerOrderDetailsScreenState extends State<ManagerOrderDetailsScreen> {
  ...
  Widget _trackingCard(double rowFont) {
    final managerDept = _normalizeDepartmentInput(widget.department);
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: _departmentTrackingTile(managerDept, rowFont),
          ),
          const SizedBox(height: 6),
          _timelineSection(),
        ],
  ...
  String _normalizeDepartmentInput(String value) {
    final v = value.trim().toUpperCase();
    if (v == 'QUALITY CONTROL' || v == 'QUALITY_CONTROL')
      return 'QUALITY_CONTROL';
    if (v == 'PACKING') return 'PACKAGING';
    // Map common names to canonical departments
    if (v.startsWith('CUT')) return 'CUTTING';
    if (v.startsWith('STITCH')) return 'STITCHING';
    if (v.startsWith('THREAD')) return 'THREADING';
    if (v.startsWith('PACK') || v == 'PACKAGING') return 'PACKAGING';
    if (v.startsWith('INSPECT') || v == 'INSPECTION') return 'INSPECTION';
